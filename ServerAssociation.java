...
import com.beanit.openiec61850.internal.mms.asn1.GetNameListResponse.ListOfIdentifier;
...
import com.beanit.openiec61850.internal.mms.asn1.ReadResponse.ListOfAccessResult;
...
public final class ServerAssociation {
  ...
  public void handleNewAssociation(AcseAssociation acseAssociation, ByteBuffer associationRequest) {
    ...
    try {
      ...
      logger.warn("warning: Error during association build up", e);
      loggerString.add("Error during association build up"+e.toString());
      return;
    }
      ...
      try {
        ...
        if (confirmedServiceRequest.getGetNameList() != null) {
          ...
          if (basicObjectClass == 9) {
            ...
            loggerString.add("Got a GetServerDirectory (MMS GetNameList[DOMAIN]) request");
            ...
            loggerString.add("Got a Get{LD|LN}Directory (MMS GetNameList[NAMED_VARIABLE]) request");
            ...
            loggerString.add("Got a GetLogicalNodeDirectory[DataSet] (MMS GetNameList[NAMED_VARIABLE_LIST]) request");
          ...
          loggerString.add("Got a GetDataDirectory/GetDataDefinition (MMS GetVariableAccessAttributes) request");
          ...
          loggerString.add("Got a Write request");
        ...
        // for Data Sets
          ...
          loggerString.add("Got a CreateDataSet request");
          ...
          loggerString.add("Got a GetDataSetDirectory request");
          ...
          loggerString.add("Got a DeleteDataSet request");
  ...
  boolean sendAnMmsPdu(MMSpdu mmsResponsePdu) {
    ...
    synchronized (reverseOStream) {
      ...
      try {
        ...
        loggerString.add("IOException while encoding MMS PDU. Closing association."+ e1.toString());
        return false;
      }
      try {
        ...
        loggerString.add("IOException while sending MMS PDU. Closing association."+ e.toString());
  ...
  private MMSpdu listenForMmsRequest(AcseAssociation acseAssociation) {
    ...
    while (true) {
      ...
      try {
        ...
        loggerString.add("Connection was closed by client.");

        return null;
      } catch (SocketTimeoutException e) {
        logger.warn("Message fragment timeout occured while receiving request. Closing association.", e);
        loggerString.add("Message fragment timeout occured while receiving request. Closing association."+ e.toString());

        return null;
      } catch (IOException e) {
        logger.warn("IOException at lower layers while listening for incoming request. Closing association.", e);
        loggerString.add("IOException at lower layers while listening for incoming request. Closing association."+ e.toString());

        return null;
      } catch (DecodingException e) {
        logger.error("Error decoding request at OSI layers.", e);
        loggerString.add("Error decoding request at OSI layers."+ e.toString());
        ...
        loggerString.add("Illegal state: message timeout while receiving request though this timeout should 0 and never be thrown"+ e.toString());
        return null;
      }
      ...
      try {
        ...
        loggerString.add("IOException decoding received MMS request PDU."+ e.toString());
      ...
      if (mmsRequestPdu.getConfirmedRequestPDU() == null) {
        if (mmsRequestPdu.getConcludeRequestPDU() != null) {
          ...
          loggerString.add("Got Conclude request, will close connection");

          return null;
        } else {
          logger.warn("Got unexpected MMS PDU, will ignore it");
          loggerString.add("Got unexpected MMS PDU, will ignore it");
          continue;
        }
      }
  ...
  private MMSpdu createServiceErrorResponse(ServiceError e, int invokeId) {
    ...
    com.beanit.openiec61850.internal.mms.asn1.ServiceError asn1ServiceError = null;

    asn1ServiceError = new com.beanit.openiec61850.internal.mms.asn1.ServiceError();
    ...
    for (String mmsReference : mmsReferences) {
      if (insertRef == true) {
        if (identifierSize > negotiatedMaxPduSize - 200) {
          ...
          loggerString.add(" ->maxMMSPduSize of " + negotiatedMaxPduSize + " Bytes reached");
  ...
  private ReadResponse handleGetDataValuesRequest(ReadRequest mmsReadRequest) throws ServiceError {
    ...
    if (mmsReadRequest.getSpecificationWithResult() == null
      ...
      synchronized (serverModel) {
        for (VariableDefs.SEQUENCE variableDef : listOfVariable) {
          ...
          if (modelNode == null) {
            ...
            loggerString.add("Got a GetDataValues request for a non existent model node.");
            ...
            if (logger.isDebugEnabled()) {
              ...
              loggerString.add("Got a GetDataValues request for node: " + modelNode);
              ...
              if (!(modelNode instanceof BasicDataAttribute)) {
                for (BasicDataAttribute bda : modelNode.getBasicDataAttributes()) {
                  ...
                  loggerString.add("sub BDA is:" + bda);
                }
              }
            }
      ...
      loggerString.add("Got a GetDataSetValues request.");
  ...
  private AccessResult getReadResult(FcModelNode modelNode) {
    ...
    if (modelNode.getFc() == Fc.CO && modelNode.getName().equals("SBO")) {
      ...
      if (ctlModelNode == null
        ...
        loggerString.add("Selecting controle DO fails because ctlModel is not set to \"sbo-with-normal-security\"");
  ...
  private WriteResponse handleSetDataValuesRequest(WriteRequest mmsWriteRequest)
    ...
    if (variableAccessSpecification.getListOfVariable() != null) {
      ...
      loggerString.add("Got a SetDataValues request.");
  ...
  private void getFirstWriteResults(
    ...
    if (writeResult == null) {
      ...
      try {
        ...
        loggerString.add("SetDataValues failed because of data missmatch."+ e.toString());
    ...
    if (fc == Fc.CO) {
      ...
      if (nodeName.equals("Oper")) {
        ...
        if (ctlModelNode == null || !(ctlModelNode instanceof BdaInt8)) {
          ...
          loggerString.add("Operatring controle DO failed because ctlModel is not set.");
        ...
        /* SBO control with normal security (operate-once or operate-many) */
          ...
          loggerString.add("SetDataValues failed because of unsupported ctlModel: " + ctlModel);
        ...
        loggerString.add("SetDataValues failed because of the operation is not allowed yet: " + modelNode.getName());
        ...
        if (nodeName.equals("RptEna")) {
          ...
          if (rptEnaNode.getValue()) {
            if (urcb.dataSet == null) {
              ...
              loggerString.add("client tried to enable RCB even though there is no configured data set");
          ...
          if ((urcb.reserved == null || urcb.reserved == this) && !urcb.enabled) {
            ...
            if (dataSetRef.isEmpty()) {
              ...
              if (dataSet != null) {
                ...
                loggerString.add("Client tried to set dataSetReference of URCB to non existant data set.");
            ...
            loggerString.add("Client tried to write RCB parameter even though URCB is reserved by other client or already enabled.");
          ...
          if ((urcb.reserved == null || urcb.reserved == this) && !urcb.enabled) {
            if (!((BdaOptFlds) modelNode).isBufferOverflow()
              ...
              loggerString.add("Client tried to write OptFlds with usupported field set to true.");
          ...
          if ((urcb.reserved == this)
            ...
            loggerString.add("Client tried to initiate a general interrogation even though URCB is not enabled by this client or general interrogation is not enabled in the trigger options.");
      ...
      for (String dsRef : dsList) {
        if (insertRef == true) {
          if (identifierSize > negotiatedMaxPduSize - 200) {
            ...
            loggerString.add("maxMMSPduSize reached");
